...
proc shouldCompleteAsFile*(commandName: string, argIndex: int): bool =
    ...
    try:
        let cmd = getCommandByName(commandName)
        let argPos = argIndex - 1  # Convert to 0-based argument index

        if argPos < cmd.arguments.len:
            return cmd.arguments[argPos].argumentType == BINARY
        elif cmd.arguments.len > 0:
            # Check if last argument (for variadic args) is BINARY
            return cmd.arguments[^1].argumentType == BINARY
    except ValueError:
        discard
